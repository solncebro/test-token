...
import { Airdrop__factory, SuperNFT__factory } from "../../typechain";
...
describe('Airdrop', () => {
  it('Full Cycle', async () => {
    ...
    const randomAddress = new Array(15).fill(0).map(() => new Wallet(randomBytes(32).toString('hex')).address);
    const merkleTree = new MerkleTree(randomAddress.concat(signer.address), keccak256, { hashLeaves: true, sortPairs: true });
    ...
    await expect(airdrop.claim(proof)).to.be.revertedWith("Airdrop: Address is not a candidate for claim")

    expect(await airdrop.claimed(guy.address)).to.eq(false)
    expect(await airdrop.canClaim(guy.address, proof)).to.eq(false)
    await expect(airdrop.connect(guy).claim(proof)).to.be.revertedWith("Airdrop: Address is not a candidate for claim")

    const badProof = merkleTree.getHexProof(keccak256(guy.address));

    expect(badProof).to.eql([]);
    await expect(airdrop.connect(guy).claim(badProof)).to.be.revertedWith("Airdrop: Address is not a candidate for claim")
  })
})